...
EnrichGT offers **more than** data frames. Please see [HTML reports (gt table)](visualize.qmd) for further visualization.

## Glance details of each module

The re-enriched object from EnrichGT supports the `$` subset operator. You can use `RStudio` or `Positron`, type `$`, and then press the `Tab` key for auto-completion, as shown in the figure.

In this example, all results haven't got any extra annotation. But `EnrichGT` supports Large language models (LLMs) based enrichment result annotations, you can refer to [large language models integration of EnrichGT](llm.qmd) page for more details. After performed LLM annotations, this step will display more information and insights about enrichment results.
...
For more details, you can can use `@` to get objects in S4. Like `result@gene_modules` returns genes in cluster.
