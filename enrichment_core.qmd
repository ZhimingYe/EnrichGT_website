...
For visualize, see [ploting ORA results](visualize.qmd) for details. You can use \`egt_plot_results()\` to view the enriched result. See [re-enrichment](re_enrichment.qmd) and [FUSING](fusing.qmd) guides for more further analysis.
...
GSEA results also support re-enrichment. Brifely, GSEA now supports two kinds of figures. For visualize, see [plotting page](visualize.qmd) for further guides. The most frequently used is the `bar plot` (by function), showing p-values and NES. And we provides two ranking figures (single figure / viewing by table) by warping plotting functions from `fgsea` - the function is called `egt_plot_gsea()`. 
